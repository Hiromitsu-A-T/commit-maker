...
- タグ: stable のみ `v<version>`（例 `v0.4.2`）を基本にし、`package.json` と一致させる。preview は原則タグを切らず、手動 workflow で公開する。
- Pre-release は VS Code Marketplace の仕様に合わせて semver の `-beta` 等を使わず、`vsce` / `ovsx` の `--pre-release` で公開する。安定版は偶数 MINOR、検証版は奇数 MINOR を基本にする（例: `0.4.x` stable → `0.5.x` preview → `0.6.x` stable）。奇数 MINOR は欠番ではなく preview 系列として扱う。
...
- 週次パッチ公開は偶数 MINOR の stable 系列のみ実行し、一時ブランチ `codex/weekly-patch-release` で PATCH version 更新、バッジ統計更新、`npm run compile`、`npm test`、local runtime smoke（Ubuntu/macOS/Windows）、Marketplace / Open VSX 公開を行い、成功後に `main` と `v<version>` タグを更新する。GitHub Actions の `GITHUB_TOKEN` で push したタグは別 workflow を起動しないため、公開処理は同 workflow 内で完結させる。
- Stable 手順（迷ったらこの順で）
  1. `package.json` の version を上げる。
  2. 変更を commit する。
  3. `v<version>` でタグを切る（例 `v0.1.4`）。
  4. `main` とタグを push する（`Publish Extension (Stable)` が自動起動）。
  5. Actions で `Publish Extension (Stable)` が成功したら Marketplace / Open VSX へ同時反映される。
- Preview 手順: `package.json` を奇数 MINOR（例 `0.5.0`）に上げ、commit/push 後に Actions から `Publish Extension (Preview)` を `workflow_dispatch` で実行する。preview ではタグを切らない。実機検証完了後、安定版は次の偶数 MINOR（例 `0.6.0`）で通常リリースする。
- 必要シークレット: `VSCE_PAT`, `OVSX_PAT`
- 処理概要: version/channel 検証 → runtime smoke（Ubuntu/macOS/Windows）→ `npm ci` → `npm run compile` → `npm test` → `vsce package --no-dependencies --no-rewrite-relative-links` → `vsce publish --packagePath ... --skip-duplicate` → `ovsx publish --skip-duplicate`。preview の場合は package/publish に `--pre-release` を付与する。完了後 `vsix/` に成果物を保存し、VSIX artifact もアップロードする。
...
## OpenAI GPT-5 系モデル仕様メモ（2026-07-22時点）
...
| ------------------- | ------------------------------------------ | --------------------------------------------------------------- | ---------------------------------------------------- | --------------------------------------------------------------------------------- | -------------------------------------------------- | ---------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| gpt-5.6-sol         | Responses / Chat / Batch                    | `/v1/responses`<br>`/v1/chat/completions`                       | Responses：`model`,`input`<br>Chat：`model`,`messages` | Responses：`reasoning.effort`<br>Chat：`reasoning_effort`                           | 対応：`none/low/medium/high/xhigh/max`（既定 `medium`）     | Responses：`text.verbosity`=`low/medium/high`<br>Chat：`verbosity`=`low/medium/high` | Responses：`max_output_tokens`<br>Chat：`max_completion_tokens`                       | GPT-5.6 の最上位。`gpt-5.6` alias はこのモデルへルーティング。1,050,000 context / 128,000 max output。                                                    | 必須：`Authorization: Bearer ...` / `Content-Type: application/json` |
| gpt-5.6-terra       | Responses / Chat / Batch                    | `/v1/responses`<br>`/v1/chat/completions`                       | Responses：`model`,`input`<br>Chat：`model`,`messages` | Responses：`reasoning.effort`<br>Chat：`reasoning_effort`                           | 対応：`none/low/medium/high/xhigh/max`（既定 `medium`）     | Responses：`text.verbosity`=`low/medium/high`<br>Chat：`verbosity`=`low/medium/high` | Responses：`max_output_tokens`<br>Chat：`max_completion_tokens`                       | 知能とコストの均衡を取る tier。1,050,000 context / 128,000 max output。                                                                                  | 必須：`Authorization: Bearer ...` / `Content-Type: application/json` |
| gpt-5.6-luna        | Responses / Chat / Batch                    | `/v1/responses`<br>`/v1/chat/completions`                       | Responses：`model`,`input`<br>Chat：`model`,`messages` | Responses：`reasoning.effort`<br>Chat：`reasoning_effort`                           | 対応：`none/low/medium/high/xhigh/max`（既定 `medium`）     | Responses：`text.verbosity`=`low/medium/high`<br>Chat：`verbosity`=`low/medium/high` | Responses：`max_output_tokens`<br>Chat：`max_completion_tokens`                       | 高頻度・コスト重視の tier。1,050,000 context / 128,000 max output。                                                                                         | 必須：`Authorization: Bearer ...` / `Content-Type: application/json` |
...
追加候補と後方互換の制約（公式仕様と 2026-07-22 の実API確認）:
- `gpt-5.5-pro`: reasoning=`medium/high/xhigh`（既定 `high`）、verbosity 3段階。
- `gpt-5.4-pro`: reasoning=`medium/high/xhigh`（既定 `medium`）、verbosity 3段階。
- `gpt-5.3-codex`: reasoning=`low/medium/high/xhigh`、verbosity 3段階。
- GPT-5.1 / 5.2 系は推奨候補へ載せない。カスタム入力の後方互換用に能力表だけ保持する。
- `gpt-5.2`: reasoning=`none/low/medium/high/xhigh`（既定 `none`）、verbosity 3段階。
- `gpt-5.2-pro`: reasoning=`medium/high/xhigh`、verbosity 3段階。
- `gpt-5.2-codex`: reasoning=`low/medium/high/xhigh`、verbosity=`medium` 固定。
- `gpt-5.1-codex-max`: reasoning=`low/medium/high/xhigh`、verbosity=`medium` 固定。
- Claude候補には active のみを載せ、deprecated/retired は削除する。
- `gemini-2.5-pro` は新規ユーザー向けAPIで利用不可のため推奨候補へ載せない。
