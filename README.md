...
[![CI](https://github.com/Hiromitsu-A-T/commit-maker/actions/workflows/ci.yml/badge.svg)](https://github.com/Hiromitsu-A-T/commit-maker/actions/workflows/ci.yml) [![VS Marketplace Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2FHiromitsu-A-T%2Fcommit-maker%2Fmain%2Fmedia%2Fbadges%2Fvs-marketplace-downloads.json&logo=visualstudiocode)](https://marketplace.visualstudio.com/items?itemName=Hiromitsu.commit-maker) [![Open VSX Downloads](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2FHiromitsu-A-T%2Fcommit-maker%2Fmain%2Fmedia%2Fbadges%2Fopen-vsx-downloads.json&logo=visualstudiocode)](https://open-vsx.org/extension/Hiromitsu/commit-maker) ![License](https://img.shields.io/badge/license-Apache--2.0-blue) ![Languages](https://img.shields.io/badge/languages-32-brightgreen)
...
Gemini / OpenAI / Claude の API キー（BYOK）に加えて、API キー不要のローカル LLM にも対応。Local では Qwen3 / Gemma 4 の GGUF モデルを選び、API 利用料なしでコミットメッセージを生成できます。
...
Use your own Gemini / OpenAI / Claude API key (BYOK), or use the Local LLM provider without an API key. Local supports Qwen3 and Gemma 4 models in GGUF format for commit message generation without cloud API charges.
...
✅ **BYOK / ローカルモデル対応** – OpenAI / Gemini / Claude はあなたの API キーで動作。Local を選ぶ場合は API キー不要で、必要な llama.cpp runtime と Qwen3 / Gemma 4 などの GGUF モデルだけを後からダウンロードします。  
(English) **BYOK / local model support** – Cloud providers use your own keys; Local needs no API key and downloads only the required llama.cpp runtime and a GGUF model such as Qwen3 or Gemma 4 when you choose it.
...
✅ **プロバイダー・モデルを自由に選択** – 同じ UI で Gemini の高速モデル、OpenAI / Claude、ローカル LLM（Qwen3 / Gemma 4 の GGUF モデル）を切り替え。コスト・速度・プライバシーを最適化できます。  
(English) **Flexible provider/model selection** – Switch between Gemini, OpenAI, Claude, and Local LLMs such as Qwen3 and Gemma 4 models in GGUF format in the same UI.
...
- **Local LLM なら API 利用料なし**: Qwen3 / Gemma 4 などの GGUF モデルをダウンロードすると PC 内のリソースで生成可能。速度は端末性能と差分サイズに依存  
  (English) **No cloud API charge with Local LLM** – Download a Qwen3 or Gemma 4 model in GGUF format and generate on your machine; speed depends on device specs and diff size
...
**2. APIキー / モデルを設定**  
上部のプロバイダーで Gemini / OpenAI / Claude / Local を選び、クラウド provider なら API キー保存、Local なら Qwen3 / Gemma 4 などのモデル選択とダウンロードを行います。API キーは SecretStorage に暗号化保存されます。  
(Choose Gemini / OpenAI / Claude / Local in the top provider selector, then save a cloud API key or select and download a Local model such as Qwen3 or Gemma 4. API keys are encrypted in SecretStorage.)

> 💡 **BYOK方式**: すべて自分のAPIキーを使用。利用料は各プロバイダーの課金体系に従います。  
> (BYOK only: bring your own keys; usage is billed by each provider)

Local を選ぶ場合、API キーは不要です。上部のモデル欄で Qwen3 / Gemma 4 などの GGUF モデルを選び、「モデルをダウンロード」で llama.cpp runtime と選択モデルを取得し、以後は PC 内で生成します。  
(For Local, no API key is required. Choose a Qwen3 or Gemma 4 model in GGUF format in the top model field, then use "Download model" to fetch the llama.cpp runtime and selected model. Generation runs on your machine.)
...
Local の GGUF モデルは 1 件あたり約 2.5 GB〜5.4 GB です。不要になった場合は上部のローカルモデル欄の「削除」から選択中モデルの本体を削除できます。
...
- **Local モデル選択**: `Qwen3-4B-Instruct-2507 Q4_K_M` / `Qwen3-4B-Thinking-2507 Q4_K_M` / `Gemma 4 E4B IT Q4_K_M` から選択
  (Local model choices: `Qwen3-4B-Instruct-2507 Q4_K_M` / `Qwen3-4B-Thinking-2507 Q4_K_M` / `Gemma 4 E4B IT Q4_K_M`)
